...
class ProcessControllerClass(object):
	...
	def __init__(self, flowDictionary, handshakeMsgReceivedFlagID):
		
		self.flowDictionary = flowDictionary
		self.handshakeMsgReceivedFlagID = handshakeMsgReceivedFlagID
	...
	def outputValuesInTaskStep(self, taskStep):
		...
		for self.componentID in self.componentsToSendInformationFor:		
			...
			if bool(globalvars.handshakeMsgReceived[self.handshakeMsgReceivedFlagID][self.componentID]) == False:
				globalvars.GlobalComs.serialWrite(str(globalvars.componentProtocolIDs[self.componentID]) + "_" + str(self.taskStepComponentData[self.componentID]) + "_" + str(self.handshakeMsgReceivedFlagID) + "\r\n")
				print "Msg send to Arduino: " + str(globalvars.componentProtocolIDs[self.componentID]) + "_" + str(self.taskStepComponentData[self.componentID]) + "_" + str(self.handshakeMsgReceivedFlagID) + "\r\n"
			else:
				globalvars.handshakeMsgReceived[self.handshakeMsgReceivedFlagID][self.componentID] = False	
		...
		if self.flowOperationOutcome == "Complete":	
			...
			if self.flowConditionOutcome == "Yes":			
				...
				if self.nextStepInThisFlowChart != "Finished":
					...
					self.flowChartStep = "1"
					globalvars.timers["EskySinceLastFinish"] = globalfxns.millis()
					self.justStarted = True
					return "Finished"
